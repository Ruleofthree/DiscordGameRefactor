...
def playerone_currentHP_less_zero(msg, pOneInfo, pTwoInfo, playerOne, playerTwo, pOneCurrentHP, pTwoCurrentHP,
    ...
    print("Inside pTwo Win")
    msg.append(pTwoInfo['name'] + " won in [color=red]"
               + str(int(rounds)) + "[/color] rounds")
    base = 100
    level = abs(pOneLevel - pTwoLevel)
    opponentLevel = int(pOneLevel / 2)
    if opponentLevel < 1:
        opponentLevel = 1
    if level == 0:
        level = 1
    if level < 5:
        ...
        if rounds > 8:
            rounds = 8
        renown = int((((base * pOneLevel) ** 1.06) + ((rounds ** 2.5) + base)))
        ...
        renown = int((((base - 25) * pOneLevel / 2) ** 1.06) + ((rounds ** 2.5) + base) / 2)

    elif 10 > level:
        msg.append("As the level difference between opponents is greater than 11, total reward is divided by "
                   "four.")
        differHP = abs(pOneCurrentHP - pTwoCurrentHP)
        if differHP > 20:
            differHP = 20
        xp = int(((base - 50) * opponentLevel) + (differHP * rounds) / 4)
        renown = int((((base - 50) * pOneLevel / 2) ** 1.06) + ((rounds ** 2.5) + base) / 4)

    if pOneInfo['level'] <= 4:
        anni = int(xp * 0.25)
        xp = xp + anni
        msg.append(pOneInfo['name'] + " has earned an [color=yellow]extra 25%[/color] 1 month anniversery bonus to xp! ")
    if pTwoInfo['level'] <= 4:
        anni = int(xp * 0.25)
        xp = xp + anni
        msg.append(pTwoInfo['name'] + " has earned an [color=yellow]extra 25%[/color] 1 month anniversery bonus to xp! ")
    ...
    msg.append(pTwoInfo['name'] + " has earned: [color=red]" + str(xp) +
               "[/color] experience points and [color=yellow] " + str(renown) + "[/color] renown.\n" +
               pOneInfo['name'] + " has earned: [color=red]" + str(int(xp / 2)) +
               "[/color] experience points and [color=yellow] " +
               str(int(renown / 2)) + "[/color] renown.")
    pTwoXP = int(pTwoInfo['currentxp'] + xp)
    pTwoRenown = int(pTwoInfo['renown'] + renown)
    pOneXP = int(pOneInfo['currentxp'] + (xp / 2))
    pOneRenown = int(pOneInfo['renown'] + (renown / 2))
    ...
    if pOneInfo['currentxp'] >= pOneInfo['nextlevel']:
        ...
        levels = [5, 10, 15, 20]
        msg.append("[color=red][b]" + pOneInfo['name'].capitalize() + " has reached level " + newLevel +
                   "![/b][/color]")
        levelFile = open(charFolder + "levelchart.json", "r", encoding="utf-8")
        levelDict = json.load(levelFile)
        levelFile.close()
